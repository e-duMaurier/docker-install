...
if [[ -f "$COMPOSE_FILE_PORTAINER" ]]; then
    ...
    curl -sSL https://raw.githubusercontent.com/e-duMaurier/docker-install/main/docker-compose_portainer-ce.yml -o docker-compose.yml >> ~/script-install.log 2>&1
fi
...
if [[ -f "$COMPOSE_FILE_NGINX" ]]; then
    ...
    echo "Local compose file not found, downloading from repository..." >> ~/script-install.log 2>&1
    curl -sSL https://raw.githubusercontent.com/e-duMaurier/docker-install/main/docker-compose_nginx-proxy-manager.yml -o docker-compose.yml >> ~/script-install.log 2>&1
fi
